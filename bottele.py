...
API_KEY = '7005030458:AAGLNPf7-eIuRj07JMqdECwiRRBacmQO9zo'
...
def connect_to_database():
  ...
  try:
    if not db_conn:
      db_conn = mysql.connector.connect(host="103.153.64.170",
                                        user="checkgdv_test",
                                        password="checkgdv_test",
                                        database="checkgdv_test")
...
def set_balance(msg):
  if msg.from_user.id == 5646550838:  # ID của người dùng quản trị
...
def start_handler(msg):
  ...
  # Tạo thông báo chào mừng và hướng dẫn sử dụng bot
    welcome_message = "Chào mừng bạn đã đến với thiên đường giải trí Tele 333 👏👏👏\n\n" \
                      " Tham gia nhóm để xem all bill kết quả : https://t.me/chanlexanhchinvn \n\n" \
...
def even_account_even(msg):
  thongbao_even = "🎊🎊🎊 Chào mừng bạn đến với thiên đường giải trí isocialmmo.fun 🎊🎊🎊\n\n📢📢📢 Rất nhiều sự kiện hấp dẫn đang chờ đón bạn 📢📢📢"
...
admin_ids = [5646550838]  #  danh sách các ID của admin
...
def luu_lich_su_dung_code(telegram_id, code, so_tien, so_du, thoi_gian):
  ...
  try:
    query = "INSERT INTO lichsucode (telegram_id, code, so_tien, so_du, thoi_gian) VALUES (%s, %s, %s, %s, %s)"
...
def withdraw_balance_bank(msg):
  try:
    ...
    another_bot_token = "6310695001:AAHecY0B84EmnM_lH8JqnkN2Eyeta20MdXc"
    another_bot_chat_id = "5646550838"  # Thay bằng chat ID phù hợp của bot khác
    requests.get(
        f"https://api.telegram.org/bot{another_bot_token}/sendMessage?chat_id={another_bot_chat_id}&text={request_message}"
    )
